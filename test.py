...
while True:
    ...
    for (x, y, w, h) in reduced_faces:
        ...
        cv2.rectangle(reduced_frame, (x, y), (x+w, y+h), (50, 50, 255), 1)
        attendance=[str(output[0]),str(timestamp)]
    
    y_offset = 70  
    x_offset = 100  
    if y_offset + reduced_frame.shape[0] <= imgBackground.shape[0] and x_offset + reduced_frame.shape[1] <= imgBackground.shape[1]:
        imgBackground[y_offset:y_offset+reduced_frame.shape[0], x_offset:x_offset+reduced_frame.shape[1]] = reduced_frame
    else:
